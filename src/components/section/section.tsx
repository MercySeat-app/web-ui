...
const sectionVariants = cva("bg-white rounded-2xl overflow-hidden flex flex-col w-full", {
  variants: {
    elevation: {
...
export function Section({
  ...
  return (
    <section {...props} className={cn(sectionVariants({ elevation, className }))}>
      ...
      {!!footer && (
        <footer className="px-4 md:px-8! py-4 md:py-6! border-t border-t-gray-25">{footer}</footer>
      )}
    </section>
  );
}
