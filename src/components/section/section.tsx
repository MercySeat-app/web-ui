...
export function Section({ title, children, footer, elevation = "elevation-0", className, ...props }: SectionProps) {
  return (
    <section {...props} className={cn(sectionVariants({ elevation, className }))}>
      ...
      {!!footer && (
        <footer className="px-8 py-6 border-t border-t-gray-25">{footer}</footer>
      )}
    </section>
  )
}