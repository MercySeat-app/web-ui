...
export interface VideoUploaderProps {
  ...
  storageProvider?: "local" | "s3" | "gcs" | "firebase";
}
...
export default function VideoUploader({
  ...
  const startUpload = async () => {
    ...
    // Append the custom name
    form.append("customName", videoName);
    form.append("provider", storageProvider);
    ...
    if (meta) {
      ...
      form.append("size", String(meta.size ?? file!.size));
    }
    ...
    try {
      ...
      const res = await axios.post(uploadUrl, form, config);
